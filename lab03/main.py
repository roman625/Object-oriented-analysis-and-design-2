...
class Unit(Entity):
    ...
    def update(self):
        ...
        # Физика отталкивания (общая для всех)
        all_units = bss.team1 + bss.team2
        for unit in all_units:
        ...
        # ЛОГИКА ПОВЕДЕНИЯ (вместо паттерна Стратегия)
        if target:
            ...
            if self.unit_type == 'melee':
                if dist > 2.0:
                    self.look_at(target)
                    new_pos = self.position + self.forward * time.dt * 5
                    new_pos.x = clamp(new_pos.x, -FIELD_BOUNDARY, FIELD_BOUNDARY)
                    new_pos.z = clamp(new_pos.z, -FIELD_BOUNDARY, FIELD_BOUNDARY)
                    self.position = new_pos
                else:
            ...
            # --- RANGED ---
                ...
                if dist > 10:
                    self.look_at(target)
                    new_pos = self.position + self.forward * time.dt * 4
                    new_pos.x = clamp(new_pos.x, -FIELD_BOUNDARY, FIELD_BOUNDARY)
                    new_pos.z = clamp(new_pos.z, -FIELD_BOUNDARY, FIELD_BOUNDARY)
                    self.position = new_pos
                elif dist < 6:
                    self.look_at(target)
                    new_pos = self.position - self.forward * time.dt * 3
                    new_pos.x = clamp(new_pos.x, -FIELD_BOUNDARY, FIELD_BOUNDARY)
                    new_pos.z = clamp(new_pos.z, -FIELD_BOUNDARY, FIELD_BOUNDARY)
                    self.position = new_pos
                if dist < 15:
            ...
            # --- TANK ---
                ...
                if dist > 3.0:
                    self.look_at(target)
                    new_pos = self.position + self.forward * time.dt * 2
                    new_pos.x = clamp(new_pos.x, -FIELD_BOUNDARY, FIELD_BOUNDARY)
                    new_pos.z = clamp(new_pos.z, -FIELD_BOUNDARY, FIELD_BOUNDARY)
                    self.position = new_pos
                else:
            ...
            # --- ASSASSIN ---
                ...
                if dist > 2.0:
                    self.look_at(target)
                    new_pos = self.position + self.forward * time.dt * 8
                    new_pos.x = clamp(new_pos.x, -FIELD_BOUNDARY, FIELD_BOUNDARY)
                    new_pos.z = clamp(new_pos.z, -FIELD_BOUNDARY, FIELD_BOUNDARY)
                    self.position = new_pos
                else:
            ...
            # --- HEALER ---
                ...
                if heal_target:
                    dist_heal = distance(self.position, heal_target.position)
                    if dist_heal > 5:
                    ...
                    # Если лечить некого, атакуем врага
                    if dist > 10:
                        self.look_at(target)
                        new_pos = self.position + self.forward * time.dt * 4
                        new_pos.x = clamp(new_pos.x, -FIELD_BOUNDARY, FIELD_BOUNDARY)
